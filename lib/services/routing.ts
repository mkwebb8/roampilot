...
export async function routeBetween(from:string,to:string,fromCoordinates?:Coordinates,toCoordinates?:Coordinates):Promise<RouteLeg>{
 ...
 const data=await fetchJson<Osrm>(`https://router.project-osrm.org/route/v1/driving/${start.longitude},${start.latitude};${end.longitude},${end.latitude}?overview=false&steps=false`);
